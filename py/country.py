...
def main(filename, src, dst, outputFileName):
    ...
    dfViz2.columns=['src_country','dst_country','value']
    dfViz3 = pd.DataFrame(c_dfOcts.stack(), columns=['value']).reset_index()
    dfViz3.columns=['src_country','dst_country','value']
    ...
    srclist = list(dfViz.src_country.unique())
    dstlist = list(dfViz.dst_country.unique())
    ...
    if len(srclist) == 1 and len(dstlist) == 1:
        ...
        c_p = figure(title="Interactive Visualization",
                   ...
                   ,tooltips=[('Source', '@src_country'), ('Destination', '@dst_country'), (valuetype, '@value')]
                   )
        ...
        c_heatmap = c_p.rect(x='src_country', y='dst_country', width=1, height=1,
                source=dfViz,
        ...
        c_heatmap2 = c_p2.rect(x='src_country', y='dst_country', width=1, height=1,
                source=dfViz2,
        ...
        c_heatmap3 = c_p3.rect(x='src_country', y='dst_country', width=1, height=1,
                source=dfViz3,
        ...
        radioBtn = RadioButtonGroup(labels=["All","Flows", "Packets", "Bytes"], active=0)#, width=100)
        ...
        var obj = findObjectByKey(df.data, 'src_country', src[parseInt(cb_obj.x)], 'dst_country', dst[parseInt(cb_obj.y)]);
        ...
        save(layout,filename=outputFileName,title='Country Level')
        return 0