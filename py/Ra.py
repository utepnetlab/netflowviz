...
class Ra():
    ...
    Ra Map Customization Parameters:
        ...
        savefile: The name of the file to save map in html to. Default = 'Map'
        ...
        lineColor: Color of line that will be drawn (can be dynamically modified using lineOptions). Default = 'green'
    ...
    def dfAddLogos(self):
        """
        User defined function to add logos to the map. If none is added there will be no logos.
        Logos must be served from the web, local files are not supported.
        Add logos for both src and dst entities into df['src_logo'] and df['dst_logo']
        Example(self):
    ...
    def lineOptions(self,df):
        """
        User defined function to add conditional coloring to lines added to the map.
        Example(self,df):
            if max(df['column_name']) > 10:
                self.lineColor = 'red'
        """
        if self.lineFunction != None:
            self.lineColor = self.lineFunction(df)
    ...
    def saveMap(self):
        ...
        """
        if '.html' not in self.savefile and '.ejs' not in self.savefile:
            self.savefile = self.savefile + '.html'   
        self.m.save(self.savefile)
    
    def createMap(self):
        """
        Calls appropriate functions to create a map from class dataframe.
        """
